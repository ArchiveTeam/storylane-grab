...
class PrepareDirectories(SimpleTask):
  ...
  * use item["item_dir"] for temporary files
  ...
  def process(self, item):
    ...
    item["user_data_json"] = "%s/storylane-user-%s.json" % (item["data_dir"], item_name)

    open("%(item_dir)s/%(warc_file_base)s.warc.gz" % item, "w").close()
    open(item["user_data_json"], "w").close()
...
pipeline = Pipeline(
  ...
  PrepareDirectories(warc_prefix="storylane.com"),
  ...
  WgetDownload([ WGET_LUA,
      ...
      "--page-requisites",
      "--span-hosts", 
      "--exclude-domains", "platform.twitter.com,api.mixpanel.com,cdn.mxpnl.com,www.google-analytics.com,connect.facebook.net",
      "--timeout", "60",
      "--tries", "20",
      "--waitretry", "5",
      "--lua-script", "storylane.lua",
      "--warc-file", ItemInterpolation("%(item_dir)s/%(warc_file_base)s"),
      "--warc-header", "operator: Archive Team",
      "--warc-header", "storylane-dld-script-version: " + VERSION,
      "--warc-header", ItemInterpolation("storylane-user: %(item_name)s"),
      ItemInterpolation("http://www.storylane.com/%(item_name)s")
    ],
    env={"USER_DATA_FILENAME": ItemInterpolation("%(user_data_json)s")},
    max_tries = 2,
...
description="The maximum number of concurrent uploads."),
  ...
  RsyncUpload(
    target = ConfigInterpolation("tracker.archiveteam.org::storylane-user-lists/%s/", downloader),
    target_source_path = ItemInterpolation("%(data_dir)s/"),
    files = [
      ItemInterpolation("%(user_data_json)s")
    ]
  ),
